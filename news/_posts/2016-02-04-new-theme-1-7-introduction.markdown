...
<div class="alert alert-note" role="alert">
Currently, the two themes are in their own repositories, which is not ideal for contributors. We're doing our best to get everything merged into the `develop` as soon as possible. It should happen over the next 2 weeks. If you want to work on the default theme, it is now relatively easy: just install it as if it were the `develop` branch and the default theme will be installed by default and set up with all the modules at the right positions.
</div>

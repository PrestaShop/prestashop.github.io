...
In my interpretation, this design strategy is opposed to data-centric designs (where _models_ reflect database elements and _services_ manipulate their data), because it focuses on modeling _business objects_ and its interactions, based on user's intent or that make sense from a business, real-world-case scenarii, and that respond to state transitions which also make sense from that standpoint. 
